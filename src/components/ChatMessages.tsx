...
import { Message, Model, PermissionRequest, ContentBlock, RefundOrderItem, BookRecommendItem } from '../types';
...
export function ChatMessages({
  ...
  return (
    <div className="flex flex-col gap-6 max-w-3xl mx-auto">
      ...
      {/* 退款确认卡片 */}
      {refundOrderList && onRefundCardClose && (
        <div className="flex gap-3 ml-12">
          <RefundConfirmCard
            orders={refundOrderList}
            onClose={onRefundCardClose}
          />
        </div>
      )}

      {/* 书籍推荐卡片 */}
      {bookRecommendList && onBookRecommendClose && hasBook && onAddBook && (
        <div className="flex gap-3 ml-12">
          <BookRecommendCard
            books={bookRecommendList}
            hasBook={hasBook}
            onAddBook={onAddBook}
          />
        </div>
